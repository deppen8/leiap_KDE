...
kde_per_production <- function(period_file, title){
  pts <- read.csv(here('data', 'periods', period_file))
  ...
  png(filename = here('outputs', 'periods', paste(title, '.png', sep='')), 
...
for (fn in list.files(here('data', 'periods'))){
